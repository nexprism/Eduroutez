...
export default function CareerCategoryForm() {
  ...
  const { mutate, isPending: isSubmitting } = useMutation({
    mutationFn: async (formData: FormData) => {
      const endpoint = isEdit
        ? `${apiUrl}/career-category/${selectedCategory._id}`
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await axiosInstance.delete(`${apiUrl}/career-category/${id}`);
      return response.data;
    },
    onSuccess: () => {
    ...
    <div className="max-w-4xl mx-auto p-6">
      <Card className="shadow-lg">
        <CardHeader className="space-y-1">
          ...
          <CardDescription>
            {isEdit ? 'Modify an existing career category' : 'Add a new category to organize your career posts'}
          </CardDescription>
        </CardHeader>
